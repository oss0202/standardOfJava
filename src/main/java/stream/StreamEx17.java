...
public class StreamEx17 {
    public static void main(String[] args) {
        ...
        // 4. 기본 분할> + 각 그룹별 탈락자 조회
        Map<Boolean,  Map<Boolean, List<MyStudent>>> stuNumByGender =
                stream
                        .collect(
                                Collectors.partitioningBy(
                                        MyStudent::isGender
                                        , Collectors.partitioningBy(
                                                student -> student.getScore() < 150
                                        )));
        stuNumByGender.forEach((aBoolean, booleanListMap) -> {
            System.out.println((aBoolean?"남학생":"여학생"));
            booleanListMap.forEach((aBoolean1, myStudents) -> {
                System.out.println("150점 " + (aBoolean1?"이상":"이하"));
